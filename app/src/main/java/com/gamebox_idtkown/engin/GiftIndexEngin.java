 ...
 */
public class GiftIndexEngin extends BaseEngin<GiftIndex> {

    private static GiftIndexEngin giftIndexEngin;

    public GiftIndexEngin(Context context) {
        super(context);
    }

    public static GiftIndexEngin getImpl(Context context) {
        if (giftIndexEngin == null) {
            synchronized (GiftIndexEngin.class) {
                giftIndexEngin = new GiftIndexEngin(context);
            }
        }
        return giftIndexEngin;
    }

    public void getGiftIndexInfo(int page, int limit, String gameName, String game_id, Callback<List<GiftIndex>>
            callback) {
    ...
    Callback<List<GiftIndex>> callback) {
        if (params == null) {
            params = new HashMap<>();
        }
        ...
        try {
            OKHttpRequest.getImpl().apost2(getUrl(), params, new OnHttpResonseListener() {
                ...
                public void onSuccess(Response response) {
                    ResultInfo<List<GiftIndex>> resultInfo = null;
                    try {
                        resultInfo = JSON.parseObject(response.body, new
                                TypeReference<ResultInfo<List<GiftIndex>>>
                                        () {
                                });
