 ...
 */
public class GoodTypeEngin extends BaseEngin<GoodType> {

    @Inject
    public GoodTypeEngin() {
    }

    public void getType(String gameid, Callback<List<GoodType>> callback) {
    ...
    Callback<List<GoodType>> callback) {
        if (params == null) {
            params = new HashMap<>();
        }
        ...
        try {
            OKHttpRequest.getImpl().apost2(getUrl(), params, new OnHttpResonseListener() {
                ...
                public void onSuccess(Response response) {
                    ResultInfo<List<GoodType>> resultInfo = null;
                    try {
                        resultInfo = JSON.parseObject(response.body, new
                                TypeReference<ResultInfo<List<GoodType>>>
                                        () {
                                });
