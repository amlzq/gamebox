 ...
 */
public class MyGiftEngin extends BaseEngin<GiftDetail> {

    private static MyGiftEngin myGiftEngin;

    public MyGiftEngin(Context context) {
        super(context);
    }


    public static MyGiftEngin getImpl(Context context) {
        if (myGiftEngin == null) {
            synchronized (MyGiftEngin.class) {
                myGiftEngin = new MyGiftEngin(context);
            }
        }
        return myGiftEngin;
    }

    public void getMyGift(int page, int limit, String game_id, Callback<List<GiftDetail>> callback) {
