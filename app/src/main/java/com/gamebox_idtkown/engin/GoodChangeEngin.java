 ...
 */
public class GoodChangeEngin extends BaseEngin<GoodChange> {

    @Inject
    public GoodChangeEngin() {
    }

    public void getList(int page, int limit, String user_id, String game_id, Callback<List<GoodChange>> callback) {
    ...
    Callback<List<GoodChange>> callback) {
        if (params == null) {
            params = new HashMap<>();
        }
        ...
        try {
            OKHttpRequest.getImpl().apost2(getUrl(), params, new OnHttpResonseListener() {
                ...
                public void onSuccess(Response response) {
                    ResultInfo<List<GoodChange>> resultInfo = null;
                    try {
                        resultInfo = JSON.parseObject(response.body, new
                                TypeReference<ResultInfo<List<GoodChange>>>
                                        () {
                                });
