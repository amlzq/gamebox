...
public class NewGameActivity extends BaseGameListActivity<GameInfo, GBActionBar> {
        ...
        TaskUtil.getImpl().runTask(new Runnable() {
            ...
            public void run() {
                bindCache(newAdapter, new Runnable() {
                    @Override
                    public void run() {
                        List<GameInfo> newGameInfos = GameInfoCache.getCache(getBaseContext(), CacheConfig.GAME_INFO_NEW);
    ...
    private void getTypeInfo() {
        GameListEngin.getImpl(this).getTypeInfo(page, limit, "new", new Callback<List<GameInfo>>() {
            @Override
            public void onSuccess(final ResultInfo<List<GameInfo>> resultInfo) {
                success(resultInfo, newAdapter, new Runnable() {
                    @Override
                    public void run() {
                        GameInfoCache.setCache(getBaseContext(), resultInfo.data, CacheConfig.GAME_INFO_NEW);
                    }
                });
            }
